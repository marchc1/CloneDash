...
public static unsafe partial class Raylib
{
	...
	public static Texture2D LoadTextureFromImage(Image image) {
		...
		if (image.Format >= Nucleus.Common.Graphics.ImageFormat.BPTC_UNORM_RGBA) {
			Texture2D texture = default;

			if ((image.Width != 0) && (image.Height != 0)) {
				uint id = loadTextureHack(image.Data, image.Width, image.Height, image.Format, image.Mipmaps);

				texture.Id = id;
			}
			else
				TraceLog(TraceLogLevel.LOG_WARNING, "IMAGE: Data is not valid to load texture");
	...
	private static unsafe uint loadTextureHack(void* data, int width, int height, ImageFormat format, int mipmapCount) {
		uint id = 0;

		OpenGL.BindTexture(GLEnum.TEXTURE_2D, 0);
		OpenGL.PixelStorei(GLEnum.UNPACK_ALIGNMENT, 1);
		OpenGL.GenTextures(1, &id);
		OpenGL.BindTexture(GLEnum.TEXTURE_2D, id);
		...
		byte* dataPtr = null;
		if (data != null) dataPtr = (byte*)data;

		for (int i = 0; i < mipmapCount; i++) {
			uint mipSize = format switch { Nucleus.Common.Graphics.ImageFormat.BPTC_UNORM_RGBA => (uint)(Math.Max(1, (mipWidth + 3) / 4) * Math.Max(1, (mipHeight + 3) / 4) * 16), _ => throw new Exception() };
			...
			if (glInternalFormat != 0) {
				if (!format.IsCompressed())
					OpenGL.TexImage2D(GLEnum.TEXTURE_2D, i, (int)glInternalFormat, mipWidth, mipHeight, 0, glFormat, glType, dataPtr);
				else
					OpenGL.CompressedTexImage2D(GLEnum.TEXTURE_2D, i, glInternalFormat, mipWidth, mipHeight, 0, (int)mipSize, dataPtr);
			}
		...
		OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_WRAP_S, (int)GLEnum.REPEAT);
		OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_WRAP_T, (int)GLEnum.REPEAT);
		OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_MAG_FILTER, (int)GLEnum.NEAREST);
		OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_MIN_FILTER, (int)GLEnum.NEAREST);

		if (mipmapCount > 1) {
			OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_MAG_FILTER, (int)GLEnum.LINEAR);
			OpenGL.TexParameteri(GLEnum.TEXTURE_2D, GLEnum.TEXTURE_MIN_FILTER, (int)GLEnum.LINEAR_MIPMAP_LINEAR);
		}
