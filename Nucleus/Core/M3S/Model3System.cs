        ...
        public static Model3 Load(string modelname, bool nocache = false) {
            ...
            if (!Cache.ContainsKey(modelAbsPath) || nocache) {
                ...
                foreach (var material in schema.Materials) {
                    ...
                    var imageInfo = schema.Images[texInfo.Index];
                ...
                unsafe {
                    cache.MeshPointers = (Mesh**)Raylib.MemAlloc(len * sizeof(Mesh));
                }
                ...
                foreach (var meshData in schema.Meshes) {
                    foreach (var primitiveData in meshData.Primitives) {
                        ...
                        ushort[] ifinal = indices.ToArray();
                        float[] vfinal = vertices.ToArray();
                        float[] nfinal = normals.ToArray();
                        float[] tfinal = texcoords.ToArray();
                        float[] wfinal = weights.ToArray();
                        byte[] cfinal = new byte[vfinal.Length * 4];
                        ...
                        unsafe {
                            ...
                            fixed (ushort* iPtr = ifinal)
                            fixed (float* vPtr = vfinal)
                            fixed (float* nPtr = nfinal)
                            fixed (float* tPtr = tfinal)
                            fixed (byte* cPtr = cfinal)
                            fixed (float* wPtr = wfinal) {
                                mesh.Indices = iPtr;
                                mesh.TriangleCount = indices.Count;
                                mesh.VertexCount = vfinal.Length;

                                mesh.Vertices = vPtr;
                                mesh.TexCoords = tPtr;
                                mesh.Normals = nPtr;
                                mesh.Colors = cPtr;
                                mesh.BoneWeights = wPtr;
                            }
                            ...
                            var meshAdr = &mesh;
                            cache.MeshPointers[index] = meshAdr;
                            ...
                            meshAdr->VaoId = Rlgl.LoadVertexArray();
                            Rlgl.EnableVertexArray(meshAdr->VaoId);

                            // Vertex position coordinates
                            void* verticesAdr = (meshAdr->AnimVertices != null) ? meshAdr->AnimVertices : meshAdr->Vertices;
                            VboId[0] = Rlgl.LoadVertexBuffer(verticesAdr, meshAdr->VertexCount * 3 * sizeof(float), dynamic);
                            Rlgl.SetVertexAttribute(0, 3, Rlgl.FLOAT, 0, 0, null); //null?
                            Rlgl.EnableVertexAttribute(0);

                            // Vertex UV coordinates
                            VboId[1] = Rlgl.LoadVertexBuffer(meshAdr->TexCoords, meshAdr->VertexCount * 2 * sizeof(float), dynamic);
                            Rlgl.SetVertexAttribute(1, 2, Rlgl.FLOAT, 0, 0, null);
                            Rlgl.EnableVertexAttribute(1);

                            // Vertex normals
                            VboId[2] = Rlgl.LoadVertexBuffer(meshAdr->Normals, meshAdr->VertexCount * 3 * sizeof(float), dynamic);
                            ...
                            VboId[3] = Rlgl.LoadVertexBuffer(meshAdr->Colors, meshAdr->VertexCount * 4 * sizeof(byte), dynamic);
                            Rlgl.SetVertexAttribute(3, 4, Rlgl.UNSIGNED_BYTE, 1, 0, null);
                            Rlgl.EnableVertexAttribute(3);

                            // Joint indices
                            VboId[4] = Rlgl.LoadVertexBuffer(jPtr, joints.Count * sizeof(int), dynamic);
                            Rlgl.EnableVertexAttribute(4);
                            Rlgl.SetVertexAttribute(4, 4, Rlgl.FLOAT, false, 0, null);

                            // Bone weights
                            VboId[5] = Rlgl.LoadVertexBuffer(meshAdr->BoneWeights, meshAdr->VertexCount * 4 * sizeof(float), dynamic);
                            Rlgl.SetVertexAttribute(5, 4, Rlgl.FLOAT, 0, 0, null);
                            Rlgl.EnableVertexAttribute(5);

                            // Indices for triangles
                            VboId[6] = Rlgl.LoadVertexBufferElement(meshAdr->Indices, meshAdr->TriangleCount * 3 * sizeof(ushort), dynamic);

                            meshAdr->VboId = VboId;

                            if (meshAdr->VaoId > 0)
                                Logs.Debug($"Mesh #[{meshAdr->VaoId}] uploaded to GPU.");
