...
namespace Nucleus.UI.Elements
{
	public class Titlebar : Panel
	{
		...
		protected override void PerformLayout(float width, float height) {
			...
			if (IValidatable.IsValid(ImageRenderer)) {
				...
				ImageRenderer.Position = TitlePos switch {
					...
					Anchor.Center => new((width / 2) - (Graphics2D.GetTextSize(Title, Graphics2D.UiFontName, TextSize).W / 2), 0),
					_ => new(0, 0),
				};
		...
		public override void Paint(float width, float height) {
			...
			Graphics2D.DrawText(pnt.X, pnt.Y, Title, Graphics2D.UiFontName, TextSize, TitlePos);
		}
	}
