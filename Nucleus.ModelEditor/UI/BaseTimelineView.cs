...
public abstract class BaseTimelineView : View
{
	...
	private void TopButtonsAndTimeInfo_PaintOverride(Element self, float width, float height) {
		...
		Vector2F frameTextSize = Graphics2D.GetTextSize(curframeText, Graphics2D.UiFontName, 20);
		...
		for (double x = xstart - widthPer; x < width; x += widthPer) {
			...
			if (closeness > (frameTextSize.X * 1.5f))
				Graphics2D.DrawText(xf, (height / 2) + 2, $"{frame}", Graphics2D.UiFontName, 20, Anchor.BottomCenter);
	...
	public static void RenderGradientFrameText(Element self, float x, float height, string text, Color color) {
		var frameTextSize = Graphics2D.GetTextSize(text, Graphics2D.UiFontName, 20);
		...
		Graphics2D.DrawText(x, (height / 2) + 2, text, Graphics2D.UiFontName, 20, Anchor.BottomCenter);
	}
