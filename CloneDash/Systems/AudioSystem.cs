...
namespace CloneDash
{
    ...
    public static class AudioSystem
    {
        ...
        public static Dictionary<string, Music> LoadedMusicTracks { get; private set; } = new();
        ...
        public static Sound PlaySound(string soundpath, float volume = 1f, float pitch = 1f, float pan = 0.5f) {
            if (!LoadedSounds.ContainsKey(soundpath))
                LoadedSounds.Add(soundpath, Raylib.LoadSound(soundpath));
        ...
        public static void UnloadMusic(string name) {
            if (!LoadedMusicTracks.ContainsKey(name))
                return;

            LoadedMusicTracks.Remove(name);
            Raylib.UnloadMusicStream(LoadedMusicTracks[name]);
        }
    }
}
