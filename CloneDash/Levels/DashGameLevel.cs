...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
	...
	internal class CD_Player_UIBar : Element
	{
		...
		public override void Paint(float width, float height) {
			...
			Graphics2D.DrawText(width / 2f, 12, $"HP: {lvl.Health}/{lvl.MaxHealth}", Graphics2D.UiFontName, 22, Anchor.Center);
			float feverRatio;
			if (lvl.InFever)
			...
			Graphics2D.DrawText(width / 2f, 32 + 12, lvl.InFever ? $"FEVER! {Math.Round(lvl.FeverTimeLeft, 2):0.00}s remaining" : $"FEVER: {Math.Round((lvl.Fever / lvl.MaxFever) * 100)}%", Graphics2D.UiFontName, 22, Anchor.Center);
	...
	internal class CD_Player_Scorebar : Element
	{
		...
		public override void Paint(float width, float height) {
			...
			//Graphics2D.DrawText(width / 2f, 32 + 48, $"AUTO", Graphics2D.UiFontName, 32, Anchor.Center);
			var lvl = Level.As<DashGameLevel>();
			Graphics2D.DrawText(width * 0.4f, 32 + 24, $"{lvl.Combo}", Graphics2D.UiFontName, (int)NMath.Remap(lvl.Conductor.Time - lvl.LastCombo, 0.2f, 0, 32, 40, clampOutput: true), Anchor.Center);
			Graphics2D.DrawText(width * 0.4f, 32 + 56, "COMBO", Graphics2D.UiFontName, 24, Anchor.Center);

			Graphics2D.DrawText(width * 0.6f, 32 + 24, $"{lvl.Score}", Graphics2D.UiFontName, 32, Anchor.Center);
			Graphics2D.DrawText(width * 0.6f, 32 + 56, "SCORE", Graphics2D.UiFontName, 24, Anchor.Center);
		}
	}
}
